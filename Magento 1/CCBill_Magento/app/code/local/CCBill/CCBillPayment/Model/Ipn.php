 ...
 * @copyright   Copyright (c) 2014-2021 CCBill (http://www.ccbill.com)
 */

/**
 * CCBill Instant Payment Notification processor model
 */
class CCBill_CCBillPayment_Model_Ipn
{
    /**
     ...
     * @var string
     */
    const DEFAULT_LOG_FILE = 'ccbill_unknown_ipn.log';
    ...
    /**
     *
     * @var CCBill_CCBillPayment_Model_Config
     */
    protected $_config = null;

    /**
     * CCBill info instance
     *
     * @var CCBill_CCBillPayment_Model_Info
     */
    ...
    /**
     * Get ipn data, send verification to CCBill, run corresponding handler
     *
     * @param array $request
    ...
    /**
     * Post back to CCBill to check whether this request is a valid one
     *
     * @param Zend_Http_Client_Adapter_Interface $httpAdapter
     */
    protected function _postBack(Zend_Http_Client_Adapter_Interface $httpAdapter)
    {
        ...
        $postbackUrl = $this->_config->getCCBillUrl();
        ...
        if ($response != 'VERIFIED') {
            ...
            throw new Exception('CCBill IPN postback failure. See ' . self::DEFAULT_LOG_FILE . ' for details.');
        }
    }
    ...
    protected function _getOrder()
    {
        if (empty($this->_order)) {
            ...
            $this->_config = Mage::getModel('ccbill/config', array($methodCode, $this->_order->getStoreId()));
    ...
    protected function _getRecurringProfile()
    {
        if (empty($this->_recurringProfile)) {
            ...
            $this->_config = Mage::getModel(
                'ccbill/config', array($methodCode, $this->_recurringProfile->getStoreId())
            );
            if (!$this->_config->isMethodActive($methodCode) || !$this->_config->isMethodAvailable()) {
                throw new Exception(sprintf('Method "%s" is not available.', $methodCode));
            }
    ...
    /**
     * Validate incoming request data, as CCBill recommends
     *
     * @throws Exception
     * @link https://cms.ccbill.com/cgi-bin/marketingweb?cmd=_render-content&content_ID=developer/e_howto_admin_IPNIntro
     */
    ...
    protected function _processOrder()
    {
        ...
        try {
            ...
            $comment = $this->_createIpnComment(Mage::helper('ccbill')->__('Note: %s', $e->getMessage()), true);
            $comment->save();
            throw $e;
        }
    }
    ...
    protected function _registerAdjustment()
    {
         ...
         */
        $message = Mage::helper('ccbill')->__('IPN "%s". A dispute has been resolved and closed. %s Transaction amount %s.', ucfirst($reasonCode), $notificationAmount, $reasonComment);
    ...
    protected function _registerDispute()
    {
         ...
         *  Add IPN comment about registered dispute
         */
        $message = Mage::helper('ccbill')->__('IPN "%s". Case type "%s". Case ID "%s" %s', ucfirst($caseType), $caseTypeLabel, $caseId, $reasonComment);
        $this->_order->addStatusHistoryComment($message)
            ->setIsCustomerNotified(false)
            ->save();
    }
    ...
    protected function _registerTransaction()
    {
        try {
            ...
            switch ($paymentStatus) {
                // paid
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_COMPLETED:
                    $this->_registerPaymentCapture(true);
                    break;

                // the holded payment was denied on CCBill side
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_DENIED:
                    $this->_registerPaymentDenial();
                    break;

                // customer attempted to pay via bank account, but failed
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_FAILED:
                    // cancel order
                    $this->_registerPaymentFailure();
                    break;

                // payment was obtained, but money were not captured yet
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_PENDING:
                    $this->_registerPaymentPending();
                    break;

                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_PROCESSED:
                    $this->_registerMasspaymentsSuccess();
                    break;

                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REVERSED:// break is intentionally omitted
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_UNREVERSED:
                    $this->_registerPaymentReversal();
                    break;

                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REFUNDED:
                    $this->_registerPaymentRefund();
                    break;

                // authorization expire/void
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_EXPIRED: // break is intentionally omitted
                case CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_VOIDED:
...
//            $comment->save();
    ...
    protected function _registerRecurringProfilePaymentCapture()
    {
        ...
        if ($invoice) {
            // notify customer
            $message = Mage::helper('ccbill')->__('Notified customer about invoice #%s.', $invoice->getIncrementId());
    ...
    protected function _registerPaymentCapture($skipFraudDetection = false)
    {
        ...
        if ($invoice && !$this->_order->getEmailSent()) {
            $this->_order->sendNewOrderEmail()->addStatusHistoryComment(
                Mage::helper('ccbill')->__('Notified customer about invoice #%s.', $invoice->getIncrementId())
            )
            ->setIsCustomerNotified(true)
            ->save();
        }
    ...
    protected function _registerPaymentRefund()
    {
        ...
        if ($creditmemo) {
            ...
            $this->_order->addStatusHistoryComment(
                Mage::helper('ccbill')->__('Notified customer about creditmemo #%s.', $creditmemo->getIncrementId())
            )
            ->setIsCustomerNotified(true)
            ->save();
        }
    ...
    protected function _registerPaymentReversal()
    {
        ...
        $orderStatus = ($paymentStatus == CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REVERSED)
            ? CCBill_CCBillPayment_Model_Info::ORDER_STATUS_REVERSED
            : CCBill_CCBillPayment_Model_Info::ORDER_STATUS_CANCELED_REVERSAL;
        /**
         * Change order status to CCBill Reversed/CCBill Cancelled Reversal if it is possible.
         */
        $message = Mage::helper('ccbill')->__('IPN "%s". %s Transaction amount %s. Transaction ID: "%s"', $this->_request['payment_status'], $reasonComment, $notificationAmount, $this->_request['txn_id']);
    ...
    public function _registerPaymentPending()
    {
        ...
        // case when was placed using CCBill standard
    ...
    protected function _createIpnComment($comment = '', $addToHistory = false)
    {
        ...
        $message = Mage::helper('ccbill')->__('IPN "%s".', $paymentStatus);
        if ($comment) {
            $message .= ' ' . $comment;
        }
        if ($addToHistory) {
    ...
    protected function _importPaymentInformation()
    {
        ...
        $from = array();
        foreach (array(
            CCBill_CCBillPayment_Model_Info::PAYER_ID,
            'payer_email' => CCBill_CCBillPayment_Model_Info::PAYER_EMAIL,
            CCBill_CCBillPayment_Model_Info::PAYER_STATUS,
            CCBill_CCBillPayment_Model_Info::ADDRESS_STATUS,
            CCBill_CCBillPayment_Model_Info::PROTECTION_EL,
            CCBill_CCBillPayment_Model_Info::PAYMENT_STATUS,
            CCBill_CCBillPayment_Model_Info::PENDING_REASON,
        ...
        if ($fraudFilters) {
            $from[CCBill_CCBillPayment_Model_Info::FRAUD_FILTERS] = $fraudFilters;
        }
         ...
         * @see CCBill_CCBillPayment_Model_Pro::importPaymentInfo()
         */
    ...
    /**
     * Filter payment status from NVP into CCBill/info format
     *
    ...
    protected function _filterPaymentStatus($ipnPaymentStatus)
    {
        switch ($ipnPaymentStatus) {
            ...
            case 'Completed': return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_COMPLETED;
            case 'Denied':    return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_DENIED;
            case 'Expired':   return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_EXPIRED;
            case 'Failed':    return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_FAILED;
            case 'Pending':   return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_PENDING;
            case 'Refunded':  return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REFUNDED;
            case 'Reversed':  return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REVERSED;
            case 'Canceled_Reversal': return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_UNREVERSED;
            case 'Processed': return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_PROCESSED;
            case 'Voided':    return CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_VOIDED;
        }
        return '';
// documented in NVP, but not documented in IPN:
//CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_NONE
//CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_INPROGRESS
//CCBill_CCBillPayment_Model_Info::PAYMENTSTATUS_REFUNDEDPART
    }
